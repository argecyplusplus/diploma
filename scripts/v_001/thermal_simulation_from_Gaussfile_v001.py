...
if __name__ == "__main__":
    ...
    # 1. Находим папку, где лежит САМ этот скрипт (т.е. scripts/v_001/)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # 2. Формируем путь к конфигу именно в этой папке
    ...
    if not os.path.exists(params_file):
        ...
        # На всякий случай проверим в текущей папке, если вдруг вы его переложили
        if os.path.exists('gauss_params.csv'):
            params_file = 'gauss_params.csv'
            print("DEBUG: Найден локальный gauss_params.csv, использую его.")
        else:
            sys.exit(1)

    # --- Параметры профиля и расчёта ---
    # ВАЖНО: L должна совпадать с параметрами из геометрии
    L = 0.51  
    N_l = 80  
    ...
    if not os.path.exists(params_file):
        print(f"ОШИБКА: Файл {params_file} не найден в директории {os.getcwd()}")
        sys.exit(1)
    ...
    # Эти параметры можно будет позже вынести в аргументы запуска sys.argv
    params = {
        'h_conv': 800.0,
        'h_cool': 400.0,
        'epsilon': 0.85,
    ...
    # --- ЗАПУСК ВЫЧИСЛЕНИЙ ---
    print(f"Начало расчета теплового слоя (v_001)... Точек: {N_l}, Шагов времени: {len(t_array)}")
